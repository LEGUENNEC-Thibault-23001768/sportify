...
class EventController
{
     ...
     * @return bool
     */
    private function sendInvitationEmail($email, $token, $event): bool
    {
     ...
     * @return void
     */
    public function sendInviteApi($eventId): void
    {
        $response = new APIResponse();
        $currentUserId = $_SESSION['user_id'];
        $currentUser = User::getUserById($currentUserId);
     ...
     * @return void
     */
    public function deleteApi($eventId): void
    {
        $response = new APIResponse();
        $currentUserId = $_SESSION['user_id'];
        $currentUser = User::getUserById($currentUserId);
     ...
     * @return null
     */
    public function join($eventId): null
    {
        $response = new APIResponse();
        $currentUserId = $_SESSION['user_id'];
        $event = Event::findEvents($eventId);
...
//        if ($event['participants_count'] >= $event['max_participants']) {
     ...
     * @return null
     */
    public function leave($eventId): null
    {
        $response = new APIResponse();
        $currentUserId = $_SESSION['user_id'];
        $event = Event::findEvents($eventId);

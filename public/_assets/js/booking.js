 ...
 window.openReservationForm = function(roomElement) {
       ...
       reservationContainer.querySelector('h3').textContent = `Réserver la salle de ${roomName}`;
        ...
        dateInput.value = today.toISOString().split('T')[0];
           ...
           try {   
            $.ajax({// fetch via appel avec url : vous pouvez debug  les reponse sur les route que je propose coté backend dans network! car coté front, c'est deja assez stable sur toutes les operations a present!
              url: `/api/court/${courtId}`,// ici ma  correction de template avec `` et on  verif que response y  correspond ( toujours mieux  d'anticiper, si erreurs, via console que une execution random :)
                  type: 'GET',
                   success: function(response) {
                    console.log("Court data:", response);
             if (response?.court){    document.getElementById('max_capacity').value = response.court.max_capacity  }
                else { document.getElementById('max_capacity').value =  "" }
                  },
                error: function(xhr, status, error) {
                       console.error("Error: " + status + " - " + error);
                      showErrorToast("Erreur lors du chargement du terrain, avec sa limite.");
                 }
            });
            $.ajax({ // J'y rajoute team id etant donné le test
                url: '/api/team?event_id=' + roomElement.dataset.eventId,
            type: 'GET',
                success: function(response) {
              ...
              teamSelect.append($('<option>', {
                        value: 'null',
                     text: 'Non' // on enlevera ces noms pour des tests + clair
                        }));
                    if (response?.teams ){
             ...
             error: function(xhr, status, error) {
                console.error("Error: " + status + " - " + error);
                 showErrorToast("Erreur lors du chargement des équipes pour la réservation de groupe ");
            }
         });
  ...
  if (clickedButton.classList.contains('selected')) {
   ...
   window.initialize = () => {
    ...
    const teamName = $('#team_name').val();
      const nbParticipants = $('#nb_participants').val();
     ...
     if (teamName  && nbParticipants){
         if( parseInt(nbParticipants,10) >  parseInt(maxCapacity,10) ){
              alert('le nombre de personne inscrit surpasse  la capacite de ce terrain ');
            return;
       }
     $.ajax({
          url: '/api/team/booking',
             type: 'POST',
           data: {...formData , team_name : teamName ,nb_participants: nbParticipants},
               success: function(response) {
                    showSuccessToast('Réservation de groupe  ajoutée avec succès!');
                      closeReservationForm();
                       loadReservations();
                     },
                    error: function(xhr, status, error) {
                      let errorMessage = "Erreur lors de l'ajout de la réservation d'équipe.";
       ...
       } else  if(teamId && teamId !== 'null')
        {
          ...
          $.ajax({
                url: '/api/team/booking',
                  type: 'POST',
                    data: {...formData , team_id : teamId},
                     success: function(response) {
                      showSuccessToast('Réservation équipe créée avec succès!');
                         closeReservationForm();
                           loadReservations();
                    },
                 error: function(xhr, status, error) {
                         let errorMessage = "Erreur lors de l'ajout de la réservation de groupe .";
                     if (xhr.responseJSON && xhr.responseJSON.data && xhr.responseJSON.data.error) {
                             errorMessage = xhr.responseJSON.data.error;
                      }
                   showErrorToast(errorMessage);
